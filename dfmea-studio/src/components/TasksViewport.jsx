...
const Pill = ({ tone, children }) => (
  <span className={`text-[11px] px-2.5 py-1 rounded-full border flex items-center gap-1.5 font-medium ${tone === "good" ? "bg-emerald-50 border-emerald-200 text-emerald-700" : tone === "bad" ? "bg-amber-50 border-amber-200 text-amber-700" : "bg-white border-zinc-200 text-zinc-600 shadow-sm"}`}>
    {children}
  </span>
);
...
export default function TasksViewport() {
  ...
  const pxPerDay = 40;
  const { timelineWidth, dayCount, getLeft, getWidth, lanesPacked } = useTimelineLayout({ range, pxPerDay, lanes, tasks: filtered });

  const split = viewMode === "timeline" ? { left: 30, right: 70 } : viewMode === "split" ? { left: 45, right: 55 } : viewMode === "tasks" ? { left: 65, right: 35 } : { left: 0, right: 100 };
      ...
      <div className="px-8 pt-8 pb-4 shrink-0">
        <div className="flex items-start justify-between gap-6 mb-6">
          <div>
            ...
            <p className="mt-1 text-sm text-zinc-500">Project • <span className="font-medium text-zinc-900">{project}</span></p>
          </div>
              ...
              <div className="min-w-max">
                <div className="sticky top-0 z-20 flex bg-white border-b border-zinc-200 h-9 shadow-[0_1px_2px_rgba(0,0,0,0.02)]">
                  ...
                  <div className="relative flex-1" style={{ width: timelineWidth }}>
                    <div className="absolute inset-0 w-full h-full" style={{ backgroundImage: "linear-gradient(to right, #f4f4f5 1px, transparent 1px)", backgroundSize: `${pxPerDay}px 100%` }} />
                    {milestones.map((m) => (
                      <div key={m.id} className={`absolute top-2 px-2 py-0.5 rounded text-[9px] font-bold text-white shadow-sm transform -translate-x-1/2 ${m.color}`} style={{ left: getLeft(m.date) }}>
                        {m.title}
                      </div>
                    ))}
                  </div>
                ...
                <div className="relative">
                  <div className="absolute inset-0 pointer-events-none" style={{ left: 208, width: timelineWidth, backgroundImage: "linear-gradient(to right, #f4f4f5 1px, transparent 1px)", backgroundSize: `${pxPerDay}px 100%` }} />
                  {lanesPacked.map(({ lane, packed, rowsCount }) => (
                    <div key={lane.id} className="flex border-b border-zinc-100 hover:bg-zinc-50/40 relative">
                      ...
                      <div className="relative py-4" style={{ height: Math.max(80, rowsCount * 34 + 32) }}>
                        {packed.map((t) => (
                          <div key={t.id} className={`absolute h-[26px] rounded-lg border border-white/20 shadow-sm flex items-center px-2.5 hover:brightness-110 hover:shadow-md hover:z-20 hover:scale-[1.02] transition-all cursor-pointer overflow-hidden ${(STATUS[t.status] || STATUS.todo).bar}`} style={{ left: getLeft(t.start), top: t.row * 32, width: getWidth(t.start, t.end) }} title={`${t.title} (${t.progress}%)`}>
